from or_datasetgen import or_train_list,or_percep_output
...
while(True) :
   ...
   flag = True 
   i = 0 
   while(i < len(or_train_list)) :
        flag = True 
        op_i =  calculateOutout(prev_wi,or_train_list[i])
        ...
        err_i = mu*(or_percep_output[i] - op_i) #percptron trining rule starts 
        j = 0 
        w_i = []
        while(j<len(prev_wi)) :
             w_i.append(round(prev_wi[j]+(err_i*or_train_list[i][j]),3))
             j+=1
